...
# ====================
# MODIFIED: MIC classification thresholds
# - Low: Exp_Log2_MIC <= 3
# - Mid: >3 且 <=5
# - High: >5
# ====================
def add_mic_group(self, target_col="Exp_Log2_MIC", low_th=3.0, high_th=5.0, group_col="MIC_Group"):
    """Create MIC activity groups from continuous Log2(MIC).

    Default thresholds (MODIFIED as per user request):
        Low    : target <= 3
        Medium : 3 < target <= 5
        High   : target > 5
    """
...
def _mic_group_tests_one(self, feature, group_col="MIC_Group"):
    """Kruskal-Wallis test across MIC groups for one feature."""
    df = self.data[[feature, group_col]].copy()
    df[feature] = pd.to_numeric(df[feature], errors="coerce")
    df = df.dropna()
    if df.empty:
        return {"KW_H": np.nan, "KW_p": np.nan, "N_total": 0, "N_low": 0, "N_mid": 0, "N_high": 0}

    groups = {}
    for g in ["Low", "Mid", "High"]:
        arr = df.loc[df[group_col] == g, feature].values.astype(float)
        groups[g] = arr

    n_low, n_mid, n_high = (len(groups["Low"]), len(groups["Mid"]), len(groups["High"]))
    n_total = n_low + n_mid + n_high

    nonempty = [v for v in groups.values() if len(v) > 0]
    if len(nonempty) < 2 or n_total < 3:
        return {"KW_H": np.nan, "KW_p": np.nan, "N_total": n_total, "N_low": n_low, "N_mid": n_mid, "N_high": n_high}

    if np.nanstd(df[feature].values.astype(float)) < 1e-12:
        return {"KW_H": np.nan, "KW_p": np.nan, "N_total": n_total, "N_low": n_low, "N_mid": n_mid, "N_high": n_high}

    H, p = kruskal(*nonempty)
    return {"KW_H": float(H), "KW_p": float(p), "N_total": n_total, "N_low": n_low, "N_mid": n_mid, "N_high": n_high}


def plot_feature_by_mic_group(self, feature, target_col="Exp_Log2_MIC", group_col="MIC_Group",
                              low_th=3.0, high_th=5.0, outdir=None):
    """Boxplot (with jitter) of one feature across MIC groups, with KW p-value annotation.

    MODIFIED thresholds: Low<=3, Mid>3<=5, High>5
    """
    if outdir is None:
    ...
    if group_col not in self.data.columns:
        self.add_mic_group(target_col=target_col, low_th=low_th, high_th=high_th, group_col=group_col)
    ...
    for i, g in enumerate(order, start=1):
        ...
        ax.scatter(xs, vals, alpha=0.45, s=18)

    ax.set_xlabel(f"{target_col} group (Low<={low_th}, Mid({low_th},{high_th}], High>{high_th})")
    ax.set_ylabel(feature)
    ax.set_title(f"{feature} across MIC groups\nKruskal-Wallis p = {pval:.3g}" if np.isfinite(pval) else f"{feature} across MIC groups")
...
def run_mic_group_analysis(self, factor_cols, target_col="Exp_Log2_MIC",
                           low_th=3.0, high_th=5.0, group_col="MIC_Group",
                           top_k=30):
    """Run MIC-group stratified analysis for all factors.

    MODIFIED: Default thresholds changed to Low<=3, Mid>3<=5, High>5
    """
    ...
    for feat in factor_cols:
        ...
        p = self.plot_feature_by_mic_group(
            ...
            low_th=low_th, high_th=high_th, outdir=plot_dir
        )
        if p:
            plot_paths[feat] = p
    ...
    tests_df["KW_q"] = np.nan
    try:
        from statsmodels.stats.multitest import multipletests
        mask = tests_df["KW_p"].notna()
        if mask.sum() > 0:
            _, qvals, _, _ = multipletests(tests_df.loc[mask, "KW_p"].values, method="fdr_bh")
            tests_df.loc[mask, "KW_q"] = qvals
    except Exception:
        pass
    ...
    top_df = tests_df.sort_values(["KW_q", "KW_p"], ascending=[True, True]).head(top_k).copy()
    ...
    with open(report_md, "w", encoding="utf-8") as f:
        f.write("# MIC-group stratified analysis report\n\n")
        f.write(f"Target: **{target_col}**\n\n")
        f.write(f"Groups: Low <= {low_th}, Mid ({low_th},{high_th}], High > {high_th}\n\n")
        f.write("## Outputs\n")
        f.write(f"- Summary stats (long, including Q10/Q90): {xlsx1}\n")
        f.write(f"- Group tests (Kruskal-Wallis + FDR): {xlsx2}\n")
        f.write(f"- Top features: {top_xlsx}\n\n")
        f.write("## Top features by FDR (KW_q)\n\n")
        if len(top_df) > 0:
            show = top_df[["Feature", "KW_H", "KW_p", "KW_q", "N_total", "N_low", "N_mid", "N_high"]]
...
# ==================== MODIFIED: Hydrophobic Face Optimization with Q10/Q90 Dynamic Ranges ====================
def analyze_hydrophobic_face_optimization(self, output_excel=None):
    ...
    MODIFIED: Target Ranges now based on Q10 and Q90 dynamic calculation

    Optimization logic:
    - For each hydrophobic face parameter, calculate mean of High activity group (Q90) and Low activity group (Q10)
    - Target Range = [Q10_Mean, Q90_Mean]
    - This ensures target range dynamically adjusts based on data

    Returns:
        DataFrame with optimization status for each parameter
    """
    hf_features = {
    ...
    # Get MIC group info (using modified thresholds: Low<=3, Mid>3<=5, High>5)
    target_col = "Exp_Log2_MIC"
    group_col = "MIC_Group"

    if group_col not in self.data.columns:
        self.add_mic_group(target_col=target_col, low_th=3.0, high_th=5.0, group_col=group_col)
    ...
    for feature, info in hf_features.items():
        ...
        # Calculate Target Range: using Low group Mean as lower bound, High group Mean as upper bound
    ...
    if output_excel:
        with pd.ExcelWriter(output_excel, engine='openpyxl') as writer:
            ...
            report += "  - Low Group: MIC <= 3 (Low activity peptides)\n"
            report += "  - High Group: MIC > 5 (High activity peptides)\n"
            report += "-"*70 + "\n\n"
...
def main():
    ...
    try:
        ...
        # MODIFIED: MIC-group stratified analysis (Low/Medium/High on Exp_Log2_MIC)
        # Thresholds: Low<=3, Mid>3<=5, High>5
        mic_outputs = analyzer.run_mic_group_analysis(
            factor_cols=factor_cols,
            target_col=target_col,
            low_th=3.0, high_th=5.0,
            group_col="MIC_Group",
            top_k=30
        )
        print("\n" + "="*70)
        print("MIC-GROUP STRATIFIED ANALYSIS OUTPUTS")
        print("="*70)
        ...
        # MODIFIED: Hydrophobic Face Optimization Analysis with Q10/Q90 Dynamic Ranges
        print("\n" + "="*70)
        print("HYDROPHOBIC FACE OPTIMIZATION (Q10/Q90 Dynamic Ranges)")
        print("="*70)
        print("\nMODIFIED: Target Ranges now calculated from Q10/Q90 group statistics")
        print("  Low Group: MIC <= 3")
        print("  High Group: MIC > 5")
